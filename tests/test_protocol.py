...
class Point(object):
    ...
    def __repr__(self):
        r = '%s.%s(%r, %r)' % (self.__module__, self.__class__.__name__,
                               self.x, self.y)
        return r
...
class ReprProtocolTestCase(ProtocolTestCase):
    ...
    def test_uses_repr_format(self):
        ...
        ENCODED = b'\t'.join((repr(KEY).encode('utf-8'),
...
class ReprValueProtocolTestCase(ProtocolTestCase):
    ...
    def test_uses_repr_format(self):
        ...
        self.assertEqual((None, VALUE), ReprValueProtocol().read(repr_bytes))
        self.assertEqual(repr_bytes, ReprValueProtocol().write(None, VALUE))
